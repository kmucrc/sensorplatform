...
const Main = () => {
    ...
    const getQueryData = async () => {
        ...
        await axios.get('https://search-crc-sensorcloud-es-r4totlztus4mhc6lxxm7nxbb6e.ap-northeast-2.es.amazonaws.com/_search', {
            params: {
        ...
        .then(function (response) {
            if(response.data.hits.hits.length === 0) alert('검색된 데이터가 존재하지 않습니다.')
            setQueryData(response.data.hits.hits);
        })
    ...
    const search = () => {
        ...
        for(var q = 0; q < queryData.length; q++) {
            var date_q = new Date(queryData[q]._source.date.replace(' ', 'T'))
            if(date_q >= startDate && date_q <= endDate) searchDataList.push(queryData[q])
        }

        //검색 데이터가 존재하지 않을 시
        if(searchDataList.length === 0) alert('검색된 데이터가 존재하지 않습니다.')

        //데이터를 시간 순으로 정렬하기
        searchDataList.sort(function(a, b) {
            return new Date(a._source.date.replace(' ', 'T')) - new Date(b._source.date.replace(' ', 'T'))
        })
        ...
        if(selectType === 'human information' ||
            ...
            if(selectFieldType[0] === 'bodytemperature') {
                ...
                for(var h = 0; h< searchDataList.length; h++) {
                    var h_date = new Date(searchDataList[h]._source.date.replace(' ', 'T'));
                    if(!isNaN(h_date)) {
                        if(searchDataList[h]._source === undefined) continue;
                        else {
                            var h_valueD1 = parseFloat(searchDataList[h]._source['bodytemperature'].value);
                            var h_valueD2 = parseFloat(searchDataList[h]._source['temperature'].value);
                            var h_valueD3 = parseFloat(searchDataList[h]._source['humidity'].value);
            ...
            // step 검색 시 (human activity > 걸음수 이동거리)
                ...
                for(var s = 0; s< searchDataList.length; s++) {
                    var s_date = new Date(searchDataList[s]._source.date.replace(' ', 'T'));
                    if(!isNaN(s_date)) {
                        if(searchDataList[s]._source === undefined) continue;
                        else {
                            var s_valueD1 = parseFloat(searchDataList[s]._source['step'].value);
                            var s_valueD2 = parseFloat(searchDataList[s]._source['distance'].value);
                ...
                var stepSec = parseInt((stepTime - (stepHour*1000*60*60) - (stepMin*1000*60)) / 1000)
            ...
            //human information 검색시
                ...
                var hi_bodytemp = []; var hi_temp = []; var hi_hba1c = []; var hi_step = []; var hi_dist = []; var hi_co2 = []; var hi_co2_10 = []
                var hi_hr = []; var hi_humidity = []; var hi_spo2 = [];
                for(var hi = 0; hi< searchDataList.length; hi++) {
                    var hi_date = new Date(searchDataList[hi]._source.date.replace(' ', 'T'));
                    if(!isNaN(hi_date)) {
                        if(searchDataList[hi]._source === undefined) continue;
                            ...
                            var hi_bodytemp_value = null; var hi_temp_value = null; var hi_humidity_value = null; var hi_step_value = null; var hi_dist_value = null; var hi_co2_value = null; 
                            var hi_hr_value = null; var hi_hba1c_value = null; var hi_spo2_value = null;
                            if(searchDataList[hi]._index === "oximetry") {
                                hi_hr_value = parseFloat(searchDataList[hi]._source['hr']?searchDataList[hi]._source['hr'].value:0);
                                hi_hba1c_value = parseFloat(searchDataList[hi]._source['hba1c'].value);
                                hi_spo2_value = parseFloat(searchDataList[hi]._source['spo2'].value);
                            }
                            if(searchDataList[hi]._index === "humidity") {
                                hi_bodytemp_value = parseFloat(searchDataList[hi]._source['bodytemperature'].value);
                                hi_temp_value = parseFloat(searchDataList[hi]._source['temperature'].value);
                                hi_humidity_value = parseFloat(searchDataList[hi]._source['humidity'].value);
                                hi_step_value = parseFloat(searchDataList[hi]._source['step'].value);
                                hi_dist_value = parseFloat(searchDataList[hi]._source['distance'].value);
                ...
                if(!isNaN(date)) {
                    if(searchDataList[j]._source[selectFieldType[0]] === undefined) continue;
                        ...
                        if(selectFieldType[0] === "gps") {
                            ...
                            valueD = parseFloat(searchDataList[j]._source[selectFieldType[0]].value);
            ...
            <div className="main-options">
                <div className="main-options-type">
                    ...
                    <div className="main-options-select">
                        <select name="group-id" id="group-id" onChange={e => setGroupId(e.target.value)}>
                            {
                                groupIdList.map((e, i) => {
                                    return (
                ...
                <div className="main-options-type">
                    ...
                    <div className="main-options-select">
                        <select name="data-type" id="data-type" onChange={e => {setSelectType(e.target.value); setChartData([]); setTableData([]);}}>
                            {
                                data.map((e, i) => {
                                    return (
                                        <option value={e.type} key={i}>{e.name}</option>
                                    )
                                })
                            }
                        </select>
                ...
                <div className="main-options-field">
                    ...
                    <div className="main-options-select">
                        <select name="data-field" id="data-field" onChange={e => {setSelectField(e.target.value); setChartData([]); setTableData([]);}} value={selectField}>
                            {
                                field.map((e, i) => {
                                    return (
                                        <option value={e.type} key={i}>{e.name}</option>
                            ...
                            <TableBody>
                                ...
                                }
                                {
                                    selectFieldType[0] === 'step' ?
                                        <TableRow>
                                            <TableCell>총 {stepTableData.step_hour}시간 {stepTableData.step_min}분 {stepTableData.step_sec}초</TableCell>
                        ...
                        <Table aria-label="simple table">
                            <TableHead>
                                <TableRow>
                                    <TableCell>날짜</TableCell>
                                    {
                                        selectFieldType[0] === 'hba1c' ? <TableCell>당화혈색소</TableCell>:
                                        selectFieldType[0] === 'hr' ? <TableCell>심박</TableCell>:
                                        selectFieldType[0] === 'spo2' ? <TableCell>산소포화도</TableCell>:
                                        selectFieldType[0] === 'gps' ? <TableCell>GPS</TableCell>:
                                        selectFieldType[0] === 'co2' ? <TableCell>이산화탄소</TableCell>:
                                        selectFieldType[0] === 'co' ? <TableCell>일산화탄소</TableCell>:
                                        selectFieldType[0] === 'no2' ? <TableCell>이산화질소</TableCell>:
                                        selectFieldType[0] === 'c2h5oh' ? <TableCell>에탄올</TableCell>:
                                        selectFieldType[0] === 'h2' ? <TableCell>수소</TableCell>:
                                        selectFieldType[0] === 'nh3' ? <TableCell>암모니아</TableCell>:
                                        selectFieldType[0] === 'ch4' ? <TableCell>메탄</TableCell>:
                                        selectFieldType[0] === 'c3h8' ? <TableCell>프로판</TableCell>:
                                        selectFieldType[0] === 'c4h10' ? <TableCell>이소부탄</TableCell>:
                                        selectFieldType[0] === 'temperature' ? <TableCell>온도</TableCell>:
                                        selectFieldType[0] === 'humidity' ? <TableCell>습도</TableCell>:
                                        selectFieldType[0] === 'barometer' ? <TableCell>기압</TableCell>:
